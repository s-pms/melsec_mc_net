...
本章仅描述 ASCII（3E 帧）相关内容，便于和 Binary 模式对照排查。

### 1. ASCII 与 Binary 报文差异

| 项目 | Binary | ASCII |
| :-- | :-- | :-- |
| 帧类型 | 3E 二进制帧 | 3E ASCII 帧 |
| 传输内容 | 原始字节 | 十六进制字符（以及 bit 场景下 0/1 字符） |
| 公共头长度 | 11 bytes | 22 chars |
| 子头 | `0x50 0x00` | `"5000"` |
| 响应子头 | `0xD0 0x00`（按协议） | `"D000"` |
| 结束码 | 2 bytes | 4 chars |
...
1. 命令：`0401`
2. 子命令：`0000`（按字）或 `0001`（按位）
3. 软元件代码：2 字符（示例：`D*`、`M*`）
4. 软元件地址：6 字符（十进制地址补零到 6 位；十六进制地址按 6 位大写十六进制）
5. 点数：4 字符（十六进制）
...
对 word 数据（short/ushort/int32/uint32/int64/uint64/float/double/string）在 ASCII 编码时，需要按“每个 16 位字高字节在前”进行编码。

示例（float `12.375f`）：

1. 内存字节（小端）：`00 00 46 41`
2. 按 16 位字交换后编码：`00 00 41 46`
3. ASCII 数据区：`00004146`

若不做该处理，第三方工具读取时会出现数值异常。
...
```text
500000FF03FF000019001014010001M*00010000011
```

读 int32（`D110`, 点数 2）请求：

```text
500000FF03FF000018001004010000D*0001100002
```

写 double（`D140`, 点数 4）请求样例：

```text
500000FF03FF000028001014010000D*0001400004ADFA5C6D454A9340
```

### 6. 远程控制指令说明

远程运行/停止/复位/读 PLC 型号属于“非批量读写”命令，虽然同样走 ASCII 3E 帧，但目标设备可能因以下原因不返回响应：

1. 仿真器未实现该命令
2. PLC 侧未开启远程控制权限
3. 当前 CPU 模式或保护参数限制

建议先用批量读写链路确认通信，再验证远程控制类命令。

### 7. Binary vs ASCII 一一对照示例（D140 写 double）

以下示例用于联调时快速核对同一业务在两种协议下的完整请求帧。
...
Binary 完整请求帧（十六进制字节流）：

```text
50 00 00 FF FF 03 00 16 00 0A 00 01 14 00 00 8C 00 00 A8 04 00 AD FA 5C 6D 45 4A 93 40
```
...
1. 命令：Binary `01 14` <-> ASCII `1401`
2. 子命令（按字）：Binary `00 00` <-> ASCII `0000`
3. 软元件：Binary `D140` 通过 `A8 + 地址字节` 表示 <-> ASCII `D*000140`
4. 点数：Binary `04 00` <-> ASCII `0004`
5. 数据：Binary 原始字节 <-> ASCII 十六进制字符

注意：

1. 对于 word 数据，ASCII 路径需按“每个 16 位字高字节在前”进行编码。
2. 若第三方工具读值异常，优先核对数据区每个 word 内部字节顺序。

### 8. Binary vs ASCII 一一对照示例（D130 写 float）

以下示例与上一节 double 对照配套，用于核对 2 word 浮点写入。
...
```text
50 00 00 FF FF 03 00 10 00 0A 00 01 14 00 00 82 00 00 A8 02 00 00 00 46 41
```

ASCII 完整请求帧（字符流）：

```text
500000FF03FF000020001014010000D*000130000200004641
```
...
1. 命令：Binary `01 14` <-> ASCII `1401`
2. 子命令（按字）：Binary `00 00` <-> ASCII `0000`
3. 软元件：Binary `D130` 通过 `A8 + 地址字节` 表示 <-> ASCII `D*000130`
4. 点数：Binary `02 00` <-> ASCII `0002`
5. 数据：Binary 原始字节 <-> ASCII 十六进制字符
